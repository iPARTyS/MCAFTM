from networks.vmamba import VSSM
...
class VMUNetV2(nn.Module):
    def __init__(self, 
                 ...
                 kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu', pretrain=True
                ):


        super().__init__()
        self.decoder = EMCAD(channels=channel, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add, lgag_ks=lgag_ks, activation=activation)
...
#             nn.Conv2d(mid_channel, num_classes, 1, 1) for _ in range(4)])
        ...
        self.segHead4 = nn.Conv2d(768, 9,  kernel_size=1)
        self.segHead3 = nn.Conv2d(384, 9,  kernel_size=1)
        self.segHead2 = nn.Conv2d(192, 9,  kernel_size=1)
        self.segHead1 = nn.Conv2d(96, 9,  kernel_size=1)
        ...
        # self.deconv6 = nn.ConvTranspose2d(9, 1, 3, stride=2, padding=1, output_padding=1)
    ...
    def forward(self, x):
        ...
        f4, f3, f2, f1 = self.decoder(f4, [f3, f2, f1])
        ...
        f4 = F.interpolate(f4, scale_factor=32, mode='bilinear')
        f3 = F.interpolate(f3, scale_factor=16, mode='bilinear')
        f2 = F.interpolate(f2, scale_factor=8, mode='bilinear')
...
#             seg_outs[i] = F.interpolate(o, scale_factor=4, mode='bilinear')
        ...
        #     out = self.segHead(f4.permute(0,3,1,2))
...
if __name__ == '__main__':
    pretrained_path = '../pre_trained_weights/vmamba_small_e238_ema-1.pth'
