...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            output1, output2, output3, output4 = model(image_batch, None)
            # out = [output1, output2, output3, output4]
            loss = 0.0
            lc1, lc2 = 0.3, 0.7  # 0.3, 0.7
            # loss_agg = 0.0
            out_all = output1 + output2 + output3 + output4


            loss_outputs1 = lc1 * ce_loss(out_all, label_batch[:].long()) + lc2 * dice_loss(out_all, label_batch,
                                                                                            softmax=True)
            loss += loss_outputs1
